...
<div class="mb-6">
    <label for="{{ $id }}"
        class="block text-sm font-medium text-gray-200 mb-2 font-poppins">{{ $label }}</label>
    <div class="relative">
        <input type="{{ $type }}" name="{{ $name }}" id="{{ $id }}"
            ...
            placeholder="Enter {{ strtolower($label) }}" value="{{ old($name, $value) }}"
            {{ $required ? 'required' : '' }} />
        ...
        @if ($type === 'password')
            <div class="absolute inset-y-0 right-0 pr-4 flex items-center cursor-pointer"
                ...
                <svg id="eye-icon-{{ $id }}"
                    class="w-5 h-5 text-gray-400 hover:text-red-400 transition-colors duration-200" fill="none"
